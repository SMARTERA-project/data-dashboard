...
if not os.path.isdir(excel_dir):
    print(f"[ERROR] Podmapa 'excel_data' ne obstaja v: {os.getcwd()}", file=sys.stderr)
    sys.exit(1)
...
def read_xls_first_sheet_with_xlrd(path: str):
    try:
        import xlrd  # priporočeno: pip install xlrd==1.2.0
    except ImportError as e:
        raise ImportError(
            "Za .xls je potreben 'xlrd==1.2.0'. Namesti z: pip install xlrd==1.2.0"
        ) from e
...
def read_first_sheet_any(path: str):
    ...
    if ext == ".xls":
        ...
        raise ValueError(f"Nepodprta končnica: {ext} ({os.path.basename(path)})")
...
def main():
    ...
    # 0) Najprej počisti obstoječe za region='bih'
    count_before = collection.count_documents({"region": "bih"})
    print(f"[CLEANUP] Najdenih {count_before} dokumentov z region='bih'.")
    if count_before > 0:
        result = collection.delete_many({"region": "bih"})
        print(f"[CLEANUP] Izbrisanih {result.deleted_count} dokumentov.")

    all_datapoints = []

    # 1) Preberi vse datoteke
    for fname in sorted(os.listdir(excel_dir)):
        ...
        try:
            ...
            print(f"[WARN] Preskočeno '{fname}': {e}", file=sys.stderr)
            continue

        all_datapoints.extend(build_datapoints_for_file(fname, df, sheet_name, run_ts))

    # 2) Shrani JSON (informativno)
    out_file = os.path.join(os.getcwd(), "datapoints.json")
    try:
        with open(out_file, "w", encoding="utf-8") as f:
            json.dump(all_datapoints, f, ensure_ascii=False, indent=2)
        print(f"[JSON] {len(all_datapoints)} zapisov → {out_file}")
    except Exception as e:
        print(f"[ERROR] Zapis JSON ni uspel: {e}", file=sys.stderr)
        sys.exit(1)

    # 3) Pripravi (ali zagotovi) unikaten indeks
    try:
    ...
    if not all_datapoints:
        print("[INFO] Ni podatkov za vstaviti v Mongo.")
        sys.exit(0)

    # 4) SAMO INSERT (brez update-ov) — preskoči duplikate
    try:
        result = collection.insert_many(all_datapoints, ordered=False)
        inserted = len(result.inserted_ids)
        print(f"[Mongo] Vstavljenih novih: {inserted}")
        ...
        print(f"[Mongo] Vstavljenih novih: {inserted}, preskočenih duplikatov: {dup_count}")
        if other_errs:
            print(f"[WARN] Drugi zapisi niso uspeli: {len(other_errs)} (prvi): {other_errs[0]}", file=sys.stderr)
